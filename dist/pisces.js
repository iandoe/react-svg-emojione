
var React = require('react');

module.exports = function() {
  return React.createElement(
	"svg",
	{ viewBox: "0 0 64 64" },
	React.createElement("circle", { cx: "32", cy: "32", fill: "#c28fef", r: "30" }),
	React.createElement("path", { d: "m45.3 45.9c-4.2-3.2-6.5-7.7-7-12.3 2.1 0 4.1 0 6.2 0 2 0 2-3.1 0-3.1-2.1 0-4.1 0-6.2 0 .4-4.6 2.7-9.1 7-12.3 1.6-1.2 0-3.9-1.6-2.7-5.2 3.9-8 9.4-8.4 15-2.1 0-4.1 0-6.2 0-.4-5.7-3.4-11.3-8.8-15.2-1.6-1.2-3.2 1.5-1.6 2.7 4.4 3.2 6.8 7.8 7.3 12.5-2.1 0-4.3 0-6.4 0-2 0-2 3.1 0 3.1 2.1 0 4.3 0 6.4 0-.4 4.7-2.8 9.3-7.2 12.5-1.6 1.2 0 3.8 1.6 2.7 5.4-3.9 8.3-9.5 8.7-15.2 2.1 0 4.1 0 6.2 0 .4 5.6 3.2 11 8.5 15 1.5 1.1 3.1-1.6 1.5-2.7z", fill: "#fff", stroke: "#fff" })
);
};
